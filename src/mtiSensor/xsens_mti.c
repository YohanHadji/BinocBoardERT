...
message_handler_ref_t inbound_handler_table[] = {
    ...
    { .id = MT_UTCTIME, .handler_fn = &xsens_internal_handle_mdata2 },
...
void xsens_mti_set_orientation( xsens_interface_t *interface, float q1, float q2, float q3, float q4) {

    xsens_packet_buffer_t packet = { 0 };

    packet.message_id = MT_SETALIGNMENTROTATION;
    packet.length = 17;
    packet.payload[0] = 0x00;

    float quatData[] = {q1, q2, q3, q4};

    for (unsigned int i = 0; i<4; i++) {

        // Split a 32 bits float into 4 parts
        union {
            float f;
            uint8_t i[4];
        } u;
        ...
        // Split quatData 32 bits floats into 4 parts in packet.payload
        packet.payload[4*i+1] = u.i[3];
        packet.payload[4*i+2] = u.i[2];
        packet.payload[4*i+3] = u.i[1];
        packet.payload[4*i+4] = u.i[0];


    }

    // packet.payload[4*i+1] = (byte(quatData[i]) >> 24) & 0xFF;
    // packet.payload[4*i+2] = (byte(quatData[i]) >> 16) & 0xFF;
    // packet.payload[4*i+3] = (byte(quatData[i]) >> 8) & 0xFF;
    // packet.payload[4*i+4] = byte(quatData[i]) & 0xFF;
...
// CONTRIBUTION BY YOHAN HADJI //  

void xsens_mti_set_output( xsens_interface_t *interface, enum XDA_TYPE_IDENTIFIER outputList[], unsigned int len, enum FREQ freq)
{
    xsens_packet_buffer_t packet = { 0 };
...
void xsens_mti_reset_factory( xsens_interface_t *interface )
{
    xsens_mti_request( interface, MT_RESTOREFACTORYDEF);
}

void xsens_mti_set_filter_profile( xsens_interface_t *interface, uint16_t profile) {
    xsens_packet_buffer_t packet = { 0 };
    
    packet.message_id = MT_SETFILTERPROFILE;
    packet.length = 2;
    // Split profile into 1 byte chunks
    packet.payload[0] = (profile >> 8) & 0xFF;
    packet.payload[1] = profile & 0xFF;

    xsens_mti_send( interface, &packet );
}

void xsens_mti_set_platform( xsens_interface_t *interface, uint16_t profile) {
    xsens_packet_buffer_t packet = { 0 };
    
    packet.message_id = MT_SETGNSSPLATFORM;
    packet.length = 2;
...
void xsens_mti_set_no_rotation( xsens_interface_t *interface, int16_t timeNoRotation) {
    xsens_packet_buffer_t packet = { 0 };
    
    packet.message_id = MT_SETNOROTATION;
    packet.length = 2;
    packet.payload[0] = (timeNoRotation >> 8) & 0xFF;
    packet.payload[1] = timeNoRotation & 0xFF;
    
    xsens_mti_send( interface, &packet );
}

void xsens_mti_set_option_flag( xsens_interface_t *interface, uint8_t flag) {
    xsens_packet_buffer_t packet = { 0 };
    
    packet.message_id = MT_SETOPTIONFLAG;
    packet.length = 8;
    packet.payload[0] = 0x00;
    packet.payload[1] = 0x00;
    packet.payload[2] = 0x00;
    packet.payload[3] = flag;
...
void xsens_mti_clear_option_flag( xsens_interface_t *interface, uint8_t flag) {
    xsens_packet_buffer_t packet = { 0 };
