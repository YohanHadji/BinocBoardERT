...
static const mdata2_decode_rules_t xid_decode_table[] = {
    ...
    { XDI_UTC_TIME, XSENS_EVT_UTC_TIME, XSENS_EVT_TYPE_TIME },
    ...
    { XDI_GNSS_PVT_DATA, XSENS_EVT_GNSS_PVT_DATA, XSENS_EVT_TYPE_GNSS },
...
void xsens_mdata2_decode_field( mdata2_packet_t *output, callback_event_t evt_cb )
{
    ...
    if( decode_rule )
    {
        ...
        switch( decode_rule->type )
        {
            ...
            case XSENS_EVT_TYPE_TIME:
                // Convert the first 4 bytes of payload to a 32-bit integer
                value.data.u4 = xsens_coalesce_32BE_32LE( &output->payload[0] );
                value.data.u2 = xsens_coalesce_16BE_16LE( &output->payload[4] );
                value.data.u1x6[0] = output->payload[6];
                value.data.u1x6[1] = output->payload[7];
                value.data.u1x6[2] = output->payload[8];
                value.data.u1x6[3] = output->payload[9];
                value.data.u1x6[4] = output->payload[10];
                value.data.u1x6[5] = output->payload[11]; 
                break;

            case XSENS_EVT_TYPE_GNSS:
                value.data.u1x6[0] = output->payload[11];
                value.data.u1x6[1] = output->payload[21];
                value.data.u1x6[2] = output->payload[22];
                value.data.u4 = xsens_coalesce_32BE_32LE( &output->payload[40] );
                break;
